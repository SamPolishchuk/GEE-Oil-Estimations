...
END_DATE = datetime(2024, 1, 31)   # End of January
...
def extract_statistics(image, storage_polygons):
    ...
    # OPTION 1: Use the current reducer syntax (generates B8_mean_stdDev, B8_mean_count)
    # This works but produces nested property names
...
def run_extraction():
    ...
    # CRITICAL FIX: The actual property names generated by combine(reducer, '', True) are:
    # - {band}_mean (from first reducer)
    # - {band}_mean_stdDev (NOT {band}_stdDev)
    # - {band}_mean_min (NOT {band}_min)
    # - {band}_mean_max (NOT {band}_max)
    # - {band}_mean_count (NOT {band}_count)
    
    task = ee.batch.Export.table.toDrive(
        collection=tank_metrics,
        description='weekly_tank_metrics_corrected',
        folder='oil_tank_monitoring',
        fileFormat='CSV',
        # Don't use selectors - export ALL properties and you can filter in post-processing
        # OR use the correct property names:
        selectors=[
            # Identifiers
            'tank_id', 'location', 'week',
            # Solar angles
            'solar_zenith_angle', 'sun_elevation',
            # Mean values (these work)
            'B2_mean', 'B3_mean', 'B4_mean', 'B8_mean', 'B11_mean',
            'shadow_index_mean', 'ndvi_mean', 'ndwi_mean',
            'brightness_mean', 'texture_contrast_mean', 'texture_entropy_mean',
            # StdDev values (CORRECTED NAMES - note the _mean_ prefix)
            'B2_mean_stdDev', 'B3_mean_stdDev', 'B4_mean_stdDev', 
            'B8_mean_stdDev', 'B11_mean_stdDev',
            'shadow_index_mean_stdDev', 'ndvi_mean_stdDev', 
            'ndwi_mean_stdDev', 'brightness_mean_stdDev',
            'texture_contrast_mean_stdDev', 'texture_entropy_mean_stdDev',
            # Min values (CORRECTED NAMES)
            'B8_mean_min', 'shadow_index_mean_min',
            # Max values (CORRECTED NAMES)
            'B8_mean_max', 'shadow_index_mean_max',
            # Count values (CORRECTED NAMES)
            'B2_mean_count', 'B3_mean_count', 'B4_mean_count', 
            'B8_mean_count', 'shadow_index_mean_count'
        ]
    )
    ...
    print("\n" + "="*60)
    print("✓ EXPORT STARTED WITH CORRECTED PROPERTY NAMES")
    print("="*60)
    print("Monitor progress:")
    print("  https://code.earthengine.google.com/ → Tasks tab")
    print("\nOutput will be saved to:")
    print("  Google Drive → oil_tank_monitoring/weekly_tank_metrics_corrected.csv")
    print("\n🔧 CRITICAL FIX APPLIED:")
    print("  The reducer combine(stdDev, '', True) generates:")
    print("    • B8_mean_stdDev (NOT B8_stdDev)")
    print("    • B8_mean_count (NOT B8_count)")
    print("    • B8_mean_min (NOT B8_min)")
    print("  Updated selectors to match actual property names!")
    print("\n📊 KEY METRICS FOR OIL PRICE PREDICTION:")
    print("  PRIMARY:")
    print("    • shadow_index_mean: Lower = darker = MORE OIL")
    print("    • shadow_index_mean_stdDev: Lower = uniform = FULL TANK")
    print("  SUPPORTING:")
    print("    • B8_mean: NIR reflectance baseline")
    print("    • B8_mean_count: Data quality (valid pixels)")
    print("  SOLAR:")
    print("    • sun_elevation: Angle from horizon (for seasonal correction)")
    print("="*60)
